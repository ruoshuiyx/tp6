...
(function($) {
    ...
    jQuery.fn.extend({
        frameSlimScroll: function(options) {
    ...
    jQuery.fn.extend({
        slimscroll: jQuery.fn.frameSlimScroll
    });
...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) == null) {
        ...
        var $title = $('<a href="javascript:void(0);">'+icon+'</a>').attr(pageIdField, pageId).addClass("menu_tab");
        ...
        if (options.content) {
            ...
            var $iframe = $("<iframe></iframe>").attr("src", options.url).css("width", "100%").attr("frameborder", "no").attr("scrolling", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);
            ...
            $.each(data, function (i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        ...
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '\',icon: \'' + item.icon + '\'});';
                        $a.attr('onclick', href);
                        $a.attr("href", item.url); //设置href虽不可跳转，但是可看到左下角链接提示
                    }
